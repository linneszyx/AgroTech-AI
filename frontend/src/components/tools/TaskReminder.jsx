...
const TaskReminder = () => {
    ...
    return (
        ...
        >
            ...
            <div className="relative bg-white   rounded-xl shadow-2xl p-10  w-full mt-5 max-w-lg text-center transition-all duration-300 transform hover:scale-105" >
            <h2 className="text-2xl font-semibold mb-4 text-green-400">Irrigation Task Reminders</h2>
            ...
            {showTaskForm && (
                <form className="mb-6" onSubmit={editingTask ? handleUpdateTask : handleTaskSubmit}>
                    <div className="mb-4">
                        <label htmlFor="crop" className="block text-green-400 mb-2">Crop:</label>
                        <input
                            type="text"
                            id="crop"
                            name="crop"
                    ...
                    <div className="mb-4">
                        <label htmlFor="date" className="block text-green-400 mb-2">Date:</label>
                        <input
                            type="date"
                            id="date"
                            name="date"
                    ...
                    <div className="mb-4">
                        <label htmlFor="time" className="block text-green-400 mb-2">Time:</label>
                        <input
                            type="time"
                            id="time"
                            name="time"
                    ...
                    <div className="mb-4">
                        <label htmlFor="to" className="block text-green-400 mb-2">To (Email):</label>
                        <input
                            type="email"
                            id="to"
                            name="to"
                    ...
                    <div className="mb-4">
                        <label htmlFor="notes" className="block text-green-400 mb-2">Notes:</label>
                        <textarea
                            id="notes"
                            name="notes"
                            value={newTask.notes}
            ...
            <div>
                {tasks.length > 0 ? (
                    <ul>
                        {tasks.map((task) => (
                            <li key={task.id} className="flex justify-between items-center mb-2 p-2 border border-green-700 rounded">
                                <div>
                                    <p className="font-semibold text-green">{task.crop}</p>
                                    <p className="text-green">{format(parseISO(task.date), 'MMMM do, yyyy')} at {task.time}</p>
                                    {task.notes && <p className="text-green italic">Notes: {task.notes}</p>}
                                </div>
                                <div>
                                    <button
